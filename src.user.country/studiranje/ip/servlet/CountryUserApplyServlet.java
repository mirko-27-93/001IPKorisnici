	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(a3c.trim().length()==0) {
			...
			try {
				...
				UserRequisit requisite = controller.getRegistrator().getUserDataLink().getRequisit(username); 
				requisite.setCountry(null);
				requisite.setCountryFlagPicture(null);
				controller.getRegistrator().getUserDataLink().updateCountryFlagPicture(username, null);
				controller.getRegistrator().getUserDataLink().updateRequisite(username, requisite);
				msg.reset();
			...
			try {
				...
				controller.getRegistrator().getUserDataLink().updateCountryFlagPicture(username, userBean.getRequisit().getCountryFlagPicture().getName());
				controller.getRegistrator().getUserDataLink().updateRequisite(username, requisite);
				msg.reset();
